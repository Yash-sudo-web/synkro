...
const Main = () => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const res = await axios.get("https://synkro-backend.vercel.app/api/user/getUser/"+currentUser);
        setUser(res.data);
      } catch (err) {
        console.log(err);
      }
  ...
  useEffect(() => {
    ...
    const addfriend = async (e) => {
      e.preventDefault();
      try {
        const res = await axios.post("https://synkro-backend.vercel.app/api/conversation", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post('https://synkro-backend.vercel.app/api/messages', message);
  ...
  useEffect(() => {
    const getUserConversations = async () => {
      try { 
        const response = await axios.get(`https://synkro-backend.vercel.app/api/conversation/${userToken}`);
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        const response = await axios.get(`https://synkro-backend.vercel.app/api/messages/${currentConvo?._id}`);
